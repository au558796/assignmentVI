...
dframe<- data.frame(filename, id, study, diagnosis, trial, mean, median, range, variance, std.dev, IQR, coeff.variation.prcnt, std.error, quantile)

#and now for something completely different...
#~~~~~~~ERROR
V = try(optimizeParam(p['f0'], p['f0'], par, min.rec = 2, max.rec = 10))
if (length(V)>1){
  radius[n]= V$radius
  delay[n]= V$delay
  embed[n]= V$emddim
...
model1 = glmer(diagnosis ~ scale(range) + (1+trial|id), data = dframe, family = 'binomial')
summary(model1)

model2 = glmer(diagnosis ~ scale(variance) + (1+trial|id), data = dframe, family = 'binomial')
summary(model2)
##3a. Is study a significant predictor in these models? What should you infer from this? Does study interact with diagnosis? What should you infer from this?

model4 = glmer(diagnosis ~ study + (1+trial|id), data = dframe, family = 'binomial')
summary(model4)
